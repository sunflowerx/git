...
import java.util.Scanner;
 public class Teacher{
	...
	public  void Teacher(long id,String name,int age){
		this.id = id;
		this.name = name;
		this.age = age;
	}
...
public class Sms {
	...
	//添加老师
	public void add( Teacher stus){
		//如果数组中的元素的个数大于等于数组长度的时候，说明数组长度不够
		if(index>=Teacher.length){
			//数组扩展
			 Teacher[] demo = new  Teacher[Teacher.length+3];
			//数组的拷贝
			System.arraycopy(stus,0,demo,0,Teacher.length);
			stus = demo;
		}
		Teacher[index++] = Teacher;// stus[index] = stu; index++;
	}
	/**
	stus = {
		{1001,terry},
			...
			switch(option){
				...
				case "3"://删除
					while(true){
						...
						System.out.println("删除成功！老师个数为："+sms.index);
					}
					break;
				case "4"://查询
					while(true){
